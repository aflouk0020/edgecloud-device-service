...
public class DeviceConfigurationVersion {
    ...
    private int pollingIntervalSeconds; private int heartbeatIntervalSeconds; private int metricsCollectionIntervalSeconds;
    ...
    public DeviceConfigurationVersion(DeviceConfiguration c,String action,String fields,UUID actor){deviceId=c.getDeviceId();version=c.getVersion();pollingIntervalSeconds=c.getPollingIntervalSeconds();heartbeatIntervalSeconds=c.getHeartbeatIntervalSeconds();metricsCollectionIntervalSeconds=c.getMetricsCollectionIntervalSeconds();environment=c.getEnvironment();apiEndpoint=c.getApiEndpoint();loggingLevel=c.getLoggingLevel();tagsJson=c.getTagsJson();this.action=action;changedFields=fields;changedBy=actor;changedAt=LocalDateTime.now();}
